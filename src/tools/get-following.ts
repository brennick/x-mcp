...
export function registerGetFollowing(server: McpServer) {
  server.tool(
    "get_following",
    "Get a list of users the specified user is following. Requires the user's numeric ID — use lookup_user first to get the ID from a username.",
    {
      user_id: z
        .string()
        .min(1)
      ...
      max_results: z
        ...
        .max(1000)
        .optional()
        .describe("Number of following to return (1-1000, default 100)"),
      user_fields: z
    ...
    async ({ user_id, max_results, user_fields }) => {
      ...
      const result = await xApiFetch(
        `users/${encodeURIComponent(user_id)}/following`,
        {
      ...
      if (!result.data || !(result.data as unknown[]).length) {
        return successResult("No following found.", result.json);
      }
